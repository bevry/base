...
actions =
	...
	prepublish: (opts,next) ->
		...
		step4 = ->
			...
			exec("#{BISCOTTO} --private --internal --name #{PACKAGE_DATA.title or PACKAGE_DATA.name} --readme README.md --output-dir #{config.BISCOTTO_OUT_PATH} #{config.BISCOTTO_SRC_PATH} - LICENSE.md HISTORY.md", {stdio:'inherit', cwd:APP_PATH}, safe next, step5)
		step5 = ->
